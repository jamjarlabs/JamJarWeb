...
describe("Scene - OnMessage", () => {
    ...
    test.each<TestTuple>([
        ["Unused message type", undefined, new TestScene(new FakeMessageBus(), 0), new Message("unused")],
        [
            "On start, no payload",
            undefined,
...
describe("Scene - AddEntity", () => {
    ...
    test.each<TestTuple>([
        ["Push entity with no existing entities", undefined, new TestScene(new FakeMessageBus(), 0), new FakeEntity(0)],
        [
...
describe("Scene - Start", () => {
    ...
    test.each<TestTuple>([
        ...
        ["Success", undefined, new TestScene(new FakeMessageBus(), 0)],
    ])("%p", (description: string, expected: Error | undefined, scene: IScene) => {
        if (expected instanceof Error) {
            expect(() => {
                scene.Start();
